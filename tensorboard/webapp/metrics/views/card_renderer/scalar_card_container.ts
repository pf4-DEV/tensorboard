...
function areSeriesEqual<T extends DataSeries>(listA: T[], listB: T[]): boolean {
  if (listA.length !== listB.length) {
    return false;
  }
  return listA.every((listAVal, index) => {
        ...
        return from(classicSmoothing(dataSeriesList, smoothing)).pipe(
          map((smoothedDataSeriesList) => {
            const dataSeriesMap = new Map(
              dataSeriesList.map((datum) => [datum.id, datum])
            );
            for (const smoothedSeries of smoothedDataSeriesList) {
              const {points: origPoints} = dataSeriesMap.get(
                smoothedSeries.id
              )!;
              dataSeriesList.push({
                id: getSmoothedSeriesId(smoothedSeries.id),
                points: smoothedSeries.points.map(({y}, index) => {
                  return {...origPoints[index], y};
                }),
              });
            }
